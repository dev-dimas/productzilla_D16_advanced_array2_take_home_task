...
let transactionLogs,
  ...
  revenueEachTransaction,
  transactionByQuantity,
  storeLocationRating,
  revenueEachLocation = [];

let omzet = { lowest: {}, highest: {}, total: 0 };
...
const setTransactionByLocation = () => {
  ...
  data.forEach((log) => {
    ...
    const locExist = result.find((log) => log.location === storeLocation);
    if (!locExist) {
      result.push({
    ...
    const index = result.findIndex((logs) => logs.location === storeLocation);
    index !== -1 && result[index].transaction.push(restData);
  });

  transactionByLocation = result;
};

const setRevenueEachTransaction = () => {
  revenueEachTransaction = data.map((log) => {
    return {
      idTransaction: log.id,
      revenue: `USD ${log.items
        .reduce((total, item) => {
...
const setTransactionByQuantity = () => {
  transactionByQuantity = data.map((log) => {
    return {
      idTransaction: log.id,
      quantity: log.items.reduce((total, item) => {
        return total + parseInt(item.quantity.$numberInt);
      }, 0),
    };
  });
};
...
const setStoreLocationRating = () => {
  ...
  data.forEach((log) => {
    const { storeLocation, customer } = log;
    const locExist = result.find((log) => log.location === storeLocation);
    if (!locExist) {
      result.push({
        location: storeLocation,
    ...
    const index = result.findIndex((logs) => logs.location === storeLocation);
    index !== -1 && result[index].ratings.push(parseInt(customer.satisfaction.$numberInt));
  });

  storeLocationRating = result.map((log) => {
    const { location, ratings } = log;
    return {
      location,
      rating: +(
...
const setRevenueEachLocation = () => {
  let result = [];

  data.forEach((log) => {
    const { storeLocation, items } = log;
    const locExist = result.find((log) => log.location === storeLocation);
    if (!locExist) {
      result.push({
        location: storeLocation,
  ...
  revenueEachLocation = result.map((log) => {
    const { location, revenue } = log;
    return {
      location,
      revenue: `USD ${revenue
        .reduce((total, income) => {
...
const setOmzet = () => {
  const result = data
    .map((log) => {
      return {
        idTransaction: log.id,
        revenue: log.items.reduce((total, item) => {
          const price = parseFloat(item.price.$numberDecimal);
          const quantity = parseInt(item.quantity.$numberInt);
          return total + price * quantity;
        }, 0),
  ...
  omzet.lowest = {
    idTransaction: result[0].idTransaction,
    revenue: `USD ${result[0].revenue.toFixed(2)}`,
  };

  omzet.highest = {
    idTransaction: result[result.length - 1].idTransaction,
    revenue: `USD ${result[result.length - 1].revenue.toFixed(2)}`,
  };

  omzet.total = `USD ${result
...
setRevenueEachTransaction();
setTransactionByQuantity();
setStoreLocationRating();
setRevenueEachLocation();
setOmzet();
...
console.log(revenueEachTransaction);
...
console.log(revenueEachLocation);
console.log('\n=== Omzet Terkecil ===');
console.log(omzet.lowest);
console.log('\n=== Omzet Terbesar ===');
console.log(omzet.highest);
console.log('\n=== Omzet Total ===');
console.log(omzet.total);
